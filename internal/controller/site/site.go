...
func RegisterHTTP(s *ghttp.Server) {
	s.Group("/api/site", func(group *ghttp.RouterGroup) {
		group.GET("/basic-setting", (*Controller).GetBasicSettingHTTP)
		group.POST("/update-basic-setting", (*Controller).UpdateBasicSettingHTTP)
	})
}
